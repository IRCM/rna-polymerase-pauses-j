...
def getGeneReads(inFile,startIndex,start,stop,offset=0):
    geneReads = []
    ...
    if offset>0:
        numLines=0
        inFile.seek(startIndex-(offset*10))
        line = inFile.readline().replace('\n', '')
        place=inFile.tell()
        addReads=[]
        while place<startIndex:
            if line =='': break
            if line[0]=='c':
                line = inFile.readline().replace('\n', '')
                continue
            reads = float(line)
            addReads.append(reads)
            numLines+=1
            line = inFile.readline().replace('\n', '')
            place=inFile.tell()
        
        geneReads = addReads[-offset:]+geneReads
    return geneReads
...
def loadGenome(genomeFilename):
    ...
    for line in open(genomeFilename):
        if line[0]=='>':
            chromosome=line[1:].replace('\n','')
            genome[chromosome] = ''
        else:
            genome[chromosome]+=line.replace('\n','')
    return genome          
    
    
def getGenePeaks(GR,window=200,fold=4,peakmin=3,tnorm_thresh=2,rounds=10):
    ...
    while i<rounds and len(peaks)>numPeaks:
        ...
        peaks+= [[index,peak] for (index,peak) in enumerate(gene_reads) if peak >thresh[index] and (tnorm[index]>tnorm_thresh or peak>peakmin)]
